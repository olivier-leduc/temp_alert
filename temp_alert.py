...
class TempAlert(object):
  ...
  def run(self):
    ...
    while not self.end:
      ...
      if Decimal(air_quality) >= 200:
        time.sleep(3)
        air_quality = self.ReadAirQuality()
      msg_time  = datetime.now(tz=pytz.timezone(("America/Los_Angeles"))).strftime('%H:%M:%S %m-%d')
      logging.info("time: %s, roomTemp: %s, freezerTemp: %s, dht22_temp: %s, humidity: %s, airquality: %s", msg_time, temp_1, temp_2, dht22_temp, humidity, air_quality)
      if flags.writetolog:
        with open(OUTPUT_FILE, "a") as log:
          log.write("{0}, {1}, {2}, {3}, {4}\n".format(msg_time, temp_1, dht22_temp, humidity, air_quality))
      if not flags.alert:  # Skip everything below
        time.sleep(2)
        continue
      ...
      if t1 - t0 > ALERT_WAIT:
        ...
        if air_quality != NO_READING:
          self.CheckAirQualityRanges(air_quality)
  ...
  def CheckTempRanges(self, room, temperature, temp_min_max):
    ...
    subject = '[Temperature Alert] : ' + room + ' too  ' + cold_or_warm + str(temperature)
    self.SendEmailAlert(subject, message)
    return True
